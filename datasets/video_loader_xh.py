...
class VideoDataset(Dataset):
    ...
    def __init__(self, dataset, seq_len=15, sample='evenly', transform=None):
        ...
        self.printed_pids = set()  # Track printed person IDs
    ...
    def __get_single_item__(self, index):
        ...
        # sanity check for the loading order
        person_id = os.path.basename(os.path.dirname(os.path.dirname(img_paths[0])))
        tracklet_id = os.path.basename(os.path.dirname(img_paths[0]))
        folder = os.path.basename(os.path.dirname(os.path.dirname(os.path.dirname(img_paths[0]))))

        if person_id not in self.printed_pids:
            # print("Folder:", folder, "Person ID:", person_id, "Tracklet: ", tracklet_id)
            self.printed_pids.add(person_id)
