...
from .clip.model_video_embed_bicubic import build_model

# built on the attention pooling, video frame posembed, ols loss, bicubic extrap of model posembed, leaarned clip m weighting
# inference: masking and flipping
...
class build_transformer(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg):
        ...
        self.bottleneck = nn.InstanceNorm1d(self.in_planes)
        self.bottleneck.apply(weights_init_kaiming)
        self.bottleneck_proj = nn.InstanceNorm1d(self.in_planes_proj)
        self.bottleneck_proj.apply(weights_init_kaiming)

        self.bottleneck_proj_temp = nn.InstanceNorm1d(self.in_planes)
        self.bottleneck_proj_temp.apply(weights_init_kaiming)

        self.bottleneck_proj_temp2 = nn.InstanceNorm1d(self.in_planes)
        ...
        # camera aware embeddings
        self.cv_embed = nn.Parameter(torch.zeros(camera_num, self.in_planes))
        trunc_normal_(self.cv_embed, std=.02)
        print('camera number is : {}'.format(camera_num))

        # video frame embedding
        self.video_embed = nn.Parameter(torch.zeros(cfg.INPUT.SEQ_LEN, self.in_planes))

        # Temporal memory module
        self.SSP = ImageSpecificPrompt()
        self.TMD = Temporal_Memory_Difusion(width=self.in_planes, layers=1, heads=12, droppath=None, T=cfg.INPUT.SEQ_LEN)
        ...
        # clip memory weighing
        self.memory_alpha = nn.Parameter(torch.tensor(0.5))  # initialize to 0.5
    ...
    def forward(self, x = None, get_image = False, cam_label= None, view_label=None, text_features2=None):
        ...
        if self.model_name == 'RN50':
            ...
            # video embed
            video_embed = self.video_embed.unsqueeze(0).repeat(B, 1, 1).view(B * T, -1)  # shape: [B*T, width]

            # model
            image_features, image_features_proj_raw = self.image_encoder(x, cv_embed, video_embed)
            ...
            img_feature = self.tap_img_feature(img_feature)  # torch.Size([16, 768])
            img_feature_proj = self.tap_img_feature_proj(img_feature_proj)  # torch.Size([16, 512])
            ###################################################
            ...
            cls_f_sp = self.tap_cls_f_sp(f_sp2)  # torch.Size([64, 768])
            cls_f_sp_tap = cls_f_sp.view(B, T, -1)
            cls_f_tp = self.tap_cls_f_tp(cls_f_sp_tap)   #
        ...
        if self.training:
            ...
            video_feature_project = self.tap_video(image_features_proj_raw2)
            alpha = torch.sigmoid(self.memory_alpha)
            text_features2 = alpha * text_features2 + (1 - alpha) * self.SSP(text_features2, video_feature_project)
            ...
            else:  # this is what is used by default
