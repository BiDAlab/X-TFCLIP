...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cv_emb = None, video_emb=None):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
        if cv_emb is not None and video_emb is not None:
            x[:,0] = x[:,0] + cv_emb + video_emb
        ...
        return x12, xproj   # self.image_encoder(x) outputs
...
def resize_pos_embed(posemb, posemb_new, height, width):
    ...
    ntok_new = posemb_new.shape[0]  # e.g., 129 or 2048

    posemb_token, posemb_grid = posemb[:1], posemb[1:]
    ntok_new -= 1

    gs_old = int(math.sqrt(len(posemb_grid)))  # e.g., 14 for 14x14
    print('Position embedding resize to height:{} width: {}'.format(height, width))

    posemb_grid = posemb_grid.reshape(1, gs_old, gs_old, -1).permute(0, 3, 1, 2)
    posemb_grid = F.interpolate(posemb_grid, size=(height, width), mode='bicubic', align_corners=False)
