...
from loss.online_label_smooth import OnlineLabelSmoothing
...
import scipy.io
import pandas as pd
from datasets.load_softbiometrics_np import (load_soft_biometrics,
                                             create_mask_matrix_gender2,
                                             create_mask_matrix_ages,
                                             create_mask_matrix_ethnicity,
                                             create_mask_matrix_body_volume)
...
def do_train_stage2(cfg,
    ...
    xent_frame = OnlineLabelSmoothing(n_classes=num_classes).cuda()
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam, target_view) in enumerate(train_loader_stage2):
            ...
            with amp.autocast(enabled=True):
                ...
                if (n_iter + 1) % log_period == 0:
                    loss1 = loss_fn.compute_loss(score1, feat, target, target_cam, logits1, isprint=True)
                else:
                    loss1 = loss_fn.compute_loss(score1, feat, target, target_cam, logits1)
            ...
            # apply gradient centralization here
        ...
        # loss function step
        loss_fn.xent.next_epoch()
        xent_frame.next_epoch()
            ...
            if cfg.MODEL.DIST_TRAIN:
                ...
                for n_iter, (img, vid, camid, camids, target_view, _) in enumerate(val_loader):
                    with torch.no_grad():
                        ...
                        feat = model(img, cam_label=camids, view_label=target_view)
                        evaluator.update((feat, vid, camid))
                cmc, mAP, _, _, _, _, _ = evaluator.compute_v2()
                logger.info("Validation Results - Epoch: {}".format(epoch))
                logger.info("mAP: {:.1%}".format(mAP))
                for r in [1, 5, 10, 20]:
                    logger.info("CMC curve, Rank-{:<3}:{:.1%}".format(r, cmc[r - 1]))
...
def do_inference_rrs_visualize(cfg, model, val_loader, num_query,
    ...
    for n_iter, (img, pids, camids, camids_batch, viewids, img_paths) in enumerate(tqdm(val_loader)):
        ...
        with torch.no_grad():
            ...
            if os.path.exists(feat_path):
                ...
                feat_1 = model(img, cam_label=camids_batch, view_label=viewids)
                feat_2 = model(F_trans.hflip(img), cam_label=camids_batch, view_label=viewids)
                feat = torch.stack([feat_1, feat_2], dim=0).mean(dim=0)
            ...
            for img_path in img_paths:
                ...
                person_id = os.path.basename(os.path.dirname(os.path.dirname(img_path[0])))

                # If n_iter < num_query, it's a query tracklet, otherwise it's a gallery tracklet
                if n_iter < num_query:
                    query_tracklet_names.append(tracklet_name)  # todo: pids is not the correct one
                    print(f"Appended ID {person_id} to query tracklet names: {tracklet_name}")
                else:
                    gallery_tracklet_names.append(tracklet_name)
                    print(f"Appended ID {person_id} to gallery tracklet names: {tracklet_name}")
    ...
    # load in the
    # soft biometric arrays
    case_name = "case1_aerial_to_ground"
    gal_softbio, _ = load_soft_biometrics(attributes_csv="datasets/AG-VPReID/attributes/" + case_name + "/gallery.csv",
                                       case_folder="datasets/AG-VPReID/" + case_name + "/gallery")
    que_softbio, _ = load_soft_biometrics(attributes_csv="datasets/AG-VPReID/attributes/" + case_name + "/query.csv",
                                       case_folder="datasets/AG-VPReID/" + case_name + "/query")

    gender_mask = create_mask_matrix_gender2(query_softbio=que_softbio, gallery_softbio=gal_softbio)
    age_mask = create_mask_matrix_ages(query_softbio=que_softbio, gallery_softbio=gal_softbio)
    ethnicity_mask = create_mask_matrix_ethnicity(query_softbio=que_softbio, gallery_softbio=gal_softbio)
    body_mask = create_mask_matrix_body_volume(query_softbio=que_softbio, gallery_softbio=gal_softbio)

    distmat = (np.array(distmat) + np.array(gender_mask) + np.array(age_mask) +
               np.array(ethnicity_mask) + np.array(body_mask))
    ...
    for paths in q_img_paths:
        ...
        print("Query folder path", folder_name)

    for paths in g_img_paths:
        # Extract folder name from the first path in the sequence
        folder_name = os.path.basename(os.path.dirname(paths[0]))
        gallery_tracklet_names.append(folder_name)
        print("Gallery folder path", folder_name)
    ...
    for query_idx in range(len(query_tracklet_names)):
        ...
        gallery_distances = [distances[idx] for idx in top_gallery_indices]  # Get corresponding distances

        # Print query and matches
        print(f"Query: {query_tracklet}")
        for i, (tracklet, dist) in enumerate(zip(gallery_tracklets, gallery_distances)):
            print(f"  Rank {i + 1}: {tracklet} (Distance: {dist:.4f})")
